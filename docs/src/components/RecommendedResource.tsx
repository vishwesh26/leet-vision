...
export default function RecommendedResource({ className = '' }: { className?: string }) {
  ...
  return (
    <div 
      className={`w-full bg-white dark:bg-[#06080e] shadow-sm rounded-xl border border-gray-200 dark:border-gray-800 my-8 overflow-hidden animate-in fade-in duration-500 ${className}`}
    >
      {/* Ad Header */}
      <div className="flex justify-between items-center bg-gray-50 dark:bg-gray-900 px-3 py-1.5 border-b border-gray-200 dark:border-gray-800">
        <div className="flex items-center gap-1.5 text-[10px] sm:text-xs font-semibold text-gray-500 dark:text-gray-400">
          <span>Ad</span>
        </div>
        <button 
      ...
      <a 
        ...
        className="flex p-4 gap-4 items-center bg-white dark:bg-[#0b0e14] hover:bg-gray-50 dark:hover:bg-[#111620] transition-colors"
      >
        <div className="flex-shrink-0 w-20 h-24 bg-white flex items-center justify-center p-1 rounded border border-gray-100 dark:border-gray-800">
          <img 
            src={product.image} 
            alt={product.title} 
        ...
        <div className="flex-1 flex flex-col gap-2 min-w-0">
          <div className="text-sm font-medium text-gray-900 dark:text-gray-100 line-clamp-3 leading-snug">
            {product.title}
          </div>
          
          <div className="inline-flex items-center gap-1.5 text-[#007185] dark:text-[#3db8cc] text-xs sm:text-sm font-semibold">
            Shop now on Amazon <ExternalLink size={12} />
          </div>
        </div>
      </a>
    </div>
