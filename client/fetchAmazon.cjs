...
(async () => {
  for (let i = 0; i < urls.length; i++) {
    ...
    try {
      ...
      const title = titleMatch ? titleMatch[1].split(':')[0].trim() : 'Amazon Product';
      
      let imgMatch = html.match(/"large":"([^"]+)"/);
      if (!imgMatch) {
          imgMatch = html.match(/data-old-hires="([^"]+)"/);
      }
      if (!imgMatch) {
          imgMatch = html.match(/id="landingImage"[^>]+src="([^"]+)"/);
      }
      ...
      console.log(`Successfully fetched ${i + 1}/${urls.length}`);
    } catch (e) {
      console.log(`Failed for ${url}: ${e.message}`);
    }
  }
  
  fs.writeFileSync('amazon_products.json', JSON.stringify(results, null, 2));
  console.log('Done.');
})();
